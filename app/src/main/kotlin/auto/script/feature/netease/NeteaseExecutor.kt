...
class NeteaseExecutor @Inject constructor(
    ...
    override fun handleAccessibilityEvent(event: AccessibilityEvent) { // 处理事件
        ...
        if (currentPackageName != APP_PACKAGE_NAME) return
...
//        }
        ...
        if (event.eventType == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED || event.eventType == AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED) {
            ...
            ScriptLogger.i(TAG, "$currentClassName, $currentState")
...
//            }
    ...
    private fun handleLightUpPuzzle(callback: (() -> Unit)? = null) {
        executeWithTimeoutRetry(
            findAction = {
                ...
                // ① 查找“看视频，点亮拼图”
                val watchVideoNode =
                    a11yServiceTool.findNodeByText(a11yServiceTool.getRootNode(), "看视频，点亮拼图")
                if (watchVideoNode != null) {
                    return@executeWithTimeoutRetry watchVideoNode
                }

                // ② 查找“已全部点亮，明天再来”
                val finishedNode = a11yServiceTool.findNodeByText(
    ...
    private fun handleReturnApp(callback: (() -> Unit)? = null) {
        ...
        // ------------------ 如果返回到 APP 主页，就重新去打开免费听 ------------------
        if (currentClassName == "com.netease.cloudmusic.activity.MainActivity") {
            driveDelayed(State.WAIT_TO_OPEN_SIDE_BAR, 1500L)
            return
        }

        // ------------------ 只要文本不是 “看视频，点亮拼图” 或者 "已全部点亮，明天再来" ，就一直返回 APP------------------
        val adNode =
            a11yServiceTool.findNodeByText(a11yServiceTool.getRootNode(), "看视频，点亮拼图")
        if (adNode != null) {
            handleLightUpPuzzle {
        ...
        val finishText =
            a11yServiceTool.findNodeByText(a11yServiceTool.getRootNode(), "已全部点亮，明天再来")
        if (finishText != null) {
            stopAutomation()
            return
        }
        ...
        // ------------------ 什么情况都不符合的时候，返回 ------------------
        val result = a11yServiceTool.performActionGlobal()
        if (result) {
            ScriptLogger.i(TAG, "A11y Service 执行返回操作成功")
            handler.postDelayed({
                handleReturnApp(callback)
            }, 1200L)
        } else {
            ScriptLogger.i(TAG, "A11y Service 执行返回操作失败")
            ScriptUtils.openApp(APP_PACKAGE_NAME)
        }
        ...
        // todo 当看完广告返回 APP 时，会弹出 com.afollestad.materialdialogs.d ，这时候 根据 com.netease.cloudmusic:id/positiveBtn 的文字判断是否够时间，如果文本是 继续观看，就点击文本，如果没有就点击 “com.netease.cloudmusic:id/md_dialog_cm_close_btn” 这个按钮
